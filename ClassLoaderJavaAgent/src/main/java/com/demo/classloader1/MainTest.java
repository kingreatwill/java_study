...
// https://blog.csdn.net/puhaiyang/article/details/78165465
public class MainTest {
    ...
    private static class ClassFileChangeListenerThread extends Thread {
        ...
        public void run() {
            try {
                File file = new File(MainTest.class.getResource("").getFile() + "Worker.class");
                long lastTime = file.lastModified();
                boolean isFirst = true;
                while (true) {
                    Thread.sleep(2000);
                    File newFile = new File(MainTest.class.getResource("").getFile() + "Worker.class");
    ...
    private static void reloadFile(File newFile, MethodExcuteThread methodExcuteThread) {
        ...
        try {
            Class<?> objClass = hotClassLoader.findClass("com.demo.classloader1.Worker");
            //把这个新的CLASS设置到另一个线程中
